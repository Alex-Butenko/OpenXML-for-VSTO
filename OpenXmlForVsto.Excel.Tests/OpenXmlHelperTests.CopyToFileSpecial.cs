...
namespace OpenXmlForVsto.Excel.Tests {
    ...
    public partial class OpenXmlHelperTests {
        [Test]
        public void CopyToFileSpecial_Test_CreatesFileForValidRange() {
            Range range = _application.Workbooks.Add().Sheets[1].Cells[1, 1];

            _tmpFile = new OpenXmlHelper().CopyToFileSpecial(range, XlPasteType.xlPasteAll);

            FileAssert.Exists(_tmpFile);
        }

        [Test]
        public void CopyToFileSpecial_Test_ThrowsNREForNullRange() {
            Assert.Throws<ArgumentNullException>(() => new OpenXmlHelper().CopyToFile(null));
        }

        [Test]
        public void CopyToFileSpecial_Test_CreatesValidFile() {
            Range range = _application.Workbooks.Add().Sheets[1].Cells[1, 1];

            _tmpFile = new OpenXmlHelper().CopyToFileSpecial(range, XlPasteType.xlPasteFormats);
        ...
        [Test]
        public void CopyToFileSpecial_Test_CreatesCorrectSheetName() {
            Range range = _application.Workbooks.Add().Sheets[1].Cells[1, 1];
        ...
        [Test]
        public void CopyToFileSpecial_Test_CopiesCorrectPosition() {
        ...
        [Test]
        public void CopyToFileSpecial_Test_CopiesCorrectData() {
            Worksheet sheet = _application.Workbooks.Add().Sheets[1];
        ...
        [Test]
        public void CopyToFileSpecial_Test_CopiesCorrectStyle() {
            Worksheet sheet = _application.Workbooks.Add().Sheets[1];
        ...
        [Test]
        public void CopyToFileSpecial_Test_CopiesStyleOnly() {
            Worksheet sheet = _application.Workbooks.Add().Sheets[1];
            Range range = sheet.Range[sheet.Cells[1, 1], sheet.Cells[2, 2]];
            ...
            using (XLWorkbook workbook = new XLWorkbook(_tmpFile)) {
                ...
                string value1 = (string)resultSheet.Cell(1, 1).Value;
                string value2 = (string)resultSheet.Cell(2, 2).Value;
                string value3 = (string)resultSheet.Cell(1, 2).Value;

                Assert.That(value1, Is.EqualTo(string.Empty));
                Assert.That(value2, Is.EqualTo(string.Empty));
                Assert.That(value3, Is.EqualTo(string.Empty));
            }
        }

        [Test]
        public void CopyToFileSpecial_Test_CopiesCorrectFormulas() {
            Worksheet sheet = _application.Workbooks.Add().Sheets[1];
            ...
            _tmpFile = new OpenXmlHelper().CopyToFileSpecial(range, XlPasteType.xlPasteFormulas);
        ...
        [Test]
        public void CopyToFileSpecial_Test_CopiesCorrectFormulas_WithMissingSource() {
            Worksheet sheet = _application.Workbooks.Add().Sheets[1];
        ...
        [Test]
        public void CopyToFileSpecial_Test_ClipboardIsEmpty() {
            Worksheet sheet = _application.Workbooks.Add().Sheets[1];
            Range range = sheet.Cells[1, 2];
            sheet.Cells[1, 1].Value = 1;
